...
def run_model(f_grid, h_grid, i_threshold, w_direction, burn_seeds):
    """Returns a tuple containing the final state of the landscape and the
    total number of cells that have been burnt after playing out the scenario
    given by the model."""
    ...
    def run_model_r(f_grid, burning_cells, burn_count):
        ...
        # iterate over the non burning cells and check for ignition
        for (i, j) in coords - set(burning_cells):
        ...
        if not burning_cells:
            return (f_grid, new_burn_count)
               
        # recursive call to simulate the next unit time frame
        return run_model_r(f_grid, burning_cells, new_burn_count)
    
    return run_model_r(f_grid, burn_seeds, 0)